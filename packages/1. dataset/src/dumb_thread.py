...
def process_dataset(dataset_path, min_val=0):
    image_path = f"{dataset_path}/images"
    images_dir = os.listdir(image_path)
    images_dir = sorted(images_dir)[min_val:]
...
parser.add_argument('min', type=int, help='The name of the dataset to process')
dataset = parser.parse_args().dataset_name
min_val = parser.parse_args().min
process_dataset(f"{PREFIX}data/mcai/airsim-data/mixed/{dataset}", min_val)